...
prepare_make_conf () {
	...
	if [ -z "$opt_init" ] && [ -s "${CHROOTDIR-}/etc/make.conf" ]; then
		...
		cat >| "$makeconf" <<EOF
CPUTYPE?=$TRGT_CODE
DEFAULT_VERSIONS= bdb=18 gcc=12 java=18 llvm=14 lua=5.4 mysql=8.0 nodejs=18 perl5=5.36 pgsql=14 php=8.1 python=3.10 python3=3.10 ruby=3.1 samba=4.16 ssl=openssl
EOF
