...
prepare_make_conf () {
	...
	if [ -z "$opt_init" ] && [ -s "${CHROOTDIR-}/etc/make.conf" ]; then
		...
		cat >| "$makeconf" <<EOF
CPUTYPE?=$TRGT_CODE
DEFAULT_VERSIONS= bdb=18 gcc=12 java=19 llvm=15 lua=5.4 mysql=8.0 nodejs=19 perl5=5.36 pgsql=15 php=8.2 python=3.11 python3=3.11 ruby=3.2 samba=4.16 ssl=openssl
EOF
